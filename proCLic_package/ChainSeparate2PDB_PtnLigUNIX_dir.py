...
#NOTE: url = urllib.urlopen(geturl) #python2
#from urllib.request import urlopen # python3
#Before running python version type:
...
try:
    ...
    print("In order for Openbabel to be imported please try typing the following\nat the command prompt:")
    print("\nexport LD_LIBRARY_PATH=/usr/local/lib\n")
    print("Then run the menu program again.")
    sys.exit(1)
...
#for file in glob.glob("*.pdb"): #Finds all files ending with .pdb, alternative method
#    if file.endswith(".pdb"):
#        print (file)
#        PDB_ref = file
#        path = (PDB_fileloc + "/" + PDB_ref)
#        print(path)
#        f = open(path)

PDB_Collect_files = [f for f in os.listdir(PDB_fileloc) if f.endswith(".pdb")]
LEN_PDB_Collect_files = len(PDB_Collect_files)
#print(LEN_PDB_Collect_files)
if LEN_PDB_Collect_files > 1:
    print("NOTE: PDB files have already been created,\nnew pdb runs will add to files already created.\n")
    PDB_ref = input("Please enter the PDB file of interest from the list below:\n" + str(PDB_Collect_files) +"\n")
    PDB_ref = re.sub('.pdb','',PDB_ref) 
    path = (PDB_fileloc + "/" + PDB_ref + ".pdb")
    print(path)
    f = open(path)
else:
    PDB_ref = PDB_Collect_files[0]
    PDB_ref = re.sub('.pdb','',PDB_ref) 
    path = (PDB_fileloc + "/" + PDB_ref + ".pdb")
    print(path)
    f = open(path)
...
###NOTE CHECKED ABOVE
### Checks that the file to write to is empty
#def CheckFileIsEmpty(save_f):
#    if os.stat(save_f).st_size:
#        #print ("The file {} has already been created".format(save_f))
#        print ("All PDB files have already been created".format(save_f))
#        sys.exit(1)
###END DEF

#Reads PDB.txt
#{This is a good check that the file selected is the correct PDB ref.}
#{This allows the user to see the protein ligand separation to allow for further decisions}               
...
def checkForConformations(conformation_dirs,conformation_dirs_match, mergeDecision):
    ...
    for i in range(len(directories)):#Checks for a dir with only 3 letters to match the four letter conf dir if the first letter is ignored
        if (len(directories[i])) == 3 and (str(directories[i]) == conformation_dirs_match):
            ...
            mergeDecision = input("Would you like to create the merged .pdb conformation files? Y/N \n")
            mergeDecision.lower()
...
#User Choice: Protein Chain save method
input("\nPress any key to continue: \n")
loop = 'T'
while loop == 'T':
    print(" ---------- MENU ---------- ")
    print("\nHow would you like the protein chains to be saved?")
    ...
    TS = str(input()).lower()  
    if (TS == 'a') or (TS == 's') or (TS == 'e') or (TS == 'p') or (TS == 'n'):
        loop = 'F'
        break
    else:
        print("Please enter a valid input: either A, S, P, E or N")
        loop = 'T'

#-----Get the PROTEIN CHAINS from the pdb files-----#
if TS == 'a':   
    SaveAllPTNChains(chain_array, chain_array_set)
...
if TS == 'e':
    SavePTNChainsPTNLigs(chain_array,chain_array_set)
    countResidues()
...
Lig_convert =input("Would you like to convert ligand chains? Y/N\n").lower()
if Lig_convert == 'y':
