...
def LoopThroughChains(directories,dirLength):
    for i in range(0,len(directories)):
        ...
        for j in range(0,len(sub_directories)):
            ...
            os.chdir(sub_directories[j]) # move into the chain directory
            next_directory = os.getcwd()
            path = next_directory
            #print(directories[i])
            ### run pro_ACT
            ...
            ## RUN proACT2
            #os.makedirs('proACT_run' + str(directories[i]))
            if os.path.isdir('proACT_run'): #need to check if exists otherwise will error
                pass
            else:
                os.mkdir('proACT_run')
            os.chdir('proACT_run')
            os.system('python /root/proACT2/proACT2.py --only_binding_site ' + PDB_ChainPath + " " + LIG_ChainPath)
            os.chdir("..")
            print("\nProACT2 run complete for: " + sub_directories[j] + "\n")
            #os.system('C:\Dale\Python\Project\DirectoryManager_ProteinChains\1a1b\proACT_run\proACT2.py --only_binding_site ' + PDB_Chain_file + " " + mol2_Chain_file)
            ##
            ## RUN the contactsLogV1.py program
            os.system('python3.3 /root/proCLic_package/ContactLogv1.py')
            ##
            ###
